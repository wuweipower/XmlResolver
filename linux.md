...
```
```bash
grep -v pattern file # -v 输出不匹配的行
grep -n pattern file # -n 显示匹配指定模式匹配的那些行的行号
grep -c pattern file # -c count 只想知道多少行含有匹配的模式
grep -e pattern -e f file # -e 指定多个匹配模式
```
